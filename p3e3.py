...
name_colors = "p3e1_Colors.txt" 

vector =[] #aca voy a guardar los datos
datos = []

#para que ya este cargado colores
...
layout = [
			[sg.Text('Color'), sg.InputCombo(COLORS, default_value=random.choice(COLORS), key='_COLORS_'),sg.Button('Añadir', key='_ADD_', pad=((120,5),10))],
			[sg.Text('X'), sg.Slider(range=(0, CANVAS[0]), orientation='h', size=(35, 20), default_value=random.randrange(0, CANVAS[0]), key='_X_')],
			[sg.Text('Y'), sg.Slider(range=(0, CANVAS[1]), orientation='h', size=(35, 20), default_value=random.randrange(0, CANVAS[1]), key='_Y_')],
...
while True:                 # Event Loop  
	...
	if event == '_ADD_':
		...
		colorpunto = val['_COLORS_']
		coordenadas = (val['_X_'],val['_Y_'])
		print('Color:', colorpunto,'Coordenadas:',coordenadas)
		
		datos.append({'color':colorpunto,'coord':coordenadas})
		vector.append(colorpunto+' '+str(coordenadas[0])+' '+str(coordenadas[1]))
		
		window.FindElement('_LIST_').Update(values = vector.copy())
		
		graph.DrawPoint(coordenadas, random.uniform(1,10), color=colorpunto)
		window.Refresh()
		window.FindElement('_SAVE_').Update(disabled = False)
		
	if event == '_BROWSE_COL_':
		name_colors = val['_BROWSE_COL_']
		...
		for colorpunto in iter(lambda: arch_col.readline(), ''):
			...
			colores.append(colorpunto)
		window.FindElement('_COLORS_').Update(values=colores.copy())
		arch_col.close()
		COLORS = colores.copy()
	if event == '_CLEAR_':
		graph.Erase()
	if event == '_SAVE_':
		...
		arch.close()
	window.FindElement('_X_').Update(value=random.randrange(0, CANVAS[0]))
	window.FindElement('_Y_').Update(value=random.randrange(0, CANVAS[1]))
	window.FindElement('_COLORS_').Update(value=random.choice(COLORS))
