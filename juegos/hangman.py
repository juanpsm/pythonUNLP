...
def dibujar(cpc,pistas,tema,ld):
	...
	print('Ya dijiste: ',cadena_separada(ld)) #Aca voy a mostrar los inputs sucesivos del jugador
...
def validar(letra,cpc,pistas,tema,ld):
...
`-._  `.  |________/\_____|    `-.'
   ...
   `._.`  '---------------'
	...
	print()
	print('             ¡¡GANASTE!!')
	print(ESPONJA.format(pal,cpc,6-cpc)) #son 7 las imagenes pero la 0 no cuenta porque ya se imprime antes de empezar a jugar
...
def main():
	...
	while jugar:
		...
		while not(fin):
			...
			clear()
			dibujar(cantidad_partes_cuerpo,pistas,tema,letras_dichas)
			#leo letra ingresada por teclado
			letra = input('Ingresa una letra: ').lower()
			#la validamos y la ingresamos al conjunto de letras jugadas
			letra = validar(letra,cantidad_partes_cuerpo,pistas,tema,letras_dichas)
			letras_dichas.add(letra)
			
			if letra in pal: #si acertó tengo que revelar las letras	
				...
				if cantidad_letras_adivinadas == len(pal):
					win(pal,cantidad_partes_cuerpo) #muestro mensaje de felicitacion
